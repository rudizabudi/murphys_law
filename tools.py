...
def cmd_positions() -> None:
    ...
    for pos in positions:
        ...
        print(
            ...
            f"{float(pos.get('fill_price', 0)):>11.2f}"
            f"{float(pos.get('notional', 0)):>10,.0f}"
            f"{'|':^5}"
            f"{pos["current_price"]:>6,.2f}"
            f"{pos["current_price"] * int(pos.get('shares', 0)):>10,.0f}"
            f"{'|':^5}"
            f"{(pos['current_price'] * int(pos.get('shares', 0)) / float(pos.get('notional', 0))) - 1:>+6,.2%}"
            ...
            f"{'|':^5}"
            f"  {(f'{qpi:.2f}' if qpi is not None else '—'):>0}"
